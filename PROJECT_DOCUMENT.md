...
- 项目版本：`ver0.3`
- 文档性质：功能设计、实施结果与维护基线
- 当前状态：ver0.1 翻译工作流、ver0.2 Electron Windows 启动器，以及 ver0.3 的网页导入、选区扩展、任务可靠性、流式译文、用量面板和界面优化均已实现
...
本文档第 1 至 24 节记录 ver0.1 翻译业务设计，第 25 节记录 ver0.2 Electron 启动器，第 26 至 30 节记录 ver0.3 的模块化、网页 ZIP、扩展、任务可靠性、流式译文和用量能力。历史章节保留原设计依据；ver0.3 仍不把大型文档 Worker Pool、云端部署或实际自动更新当作已实现功能。
...
ver0.3 将 ver0.2 的启动器和本地翻译服务整理为可 review 的模块化基线，并补齐网页内容的导入/导出闭环：

- `browser-extension/` 负责从当前网页选区生成标准 ZIP；扩展不接触 API Key、不携带 Cookie，也不执行任意命令。
- `POST /api/import/web-package` 负责安全读取 ZIP；`document.json` 是唯一规范来源，资源先暂存，校验通过后才原子提交。
- `TranslationProvider`、`RuleProposalProvider`、SSE 读取器、Token 统计和余额查询均通过应用层接入，HTTP 路由不直接请求 DeepSeek。
- 翻译任务继续使用 SQLite 持久化、revision CAS、worker 租约和心跳；当前仍是单任务内顺序翻译，不是 Worker Pool。
- DeepSeek 流式响应只把当前未完成译文放在内存并通过 SSE 推送；分段完成后按既有校验和事务边界写入数据库。
- 当前版本号为 `0.3.0`；标准网页 ZIP 的单资源上限为 20 MB，直接图片上传接口仍为 8 MB。

### 31.1 兼容性与范围

现有规则集、文本导入、剪贴板 HTML、图片顺序、翻译任务、TXT/DOCX/EPUB 导出和 Electron 管理端口均保持兼容。ver0.3 不新增文档持久化表，不改现有 API 请求字段，不实现大型文档队列、云端部署、浏览器扩展后台同步或自动更新安装。

### 31.2 验证基线

- Node.js 自动化测试基线：94 项；ZIP 扩展和导入专项测试：21 项。
- 已覆盖标准 ZIP 合法/恶意输入、SVG 隔离、资源原格式、选区顺序、规则候选校验、翻译竞态、SSE 增量、Token 用量和余额查询失败隔离。
- Windows 安装包和浏览器扩展均可独立构建；构建产物不包含 `data/`、主密钥、用户图片或日志。
