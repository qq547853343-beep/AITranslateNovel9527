...
export async function createDocxExport({ blocks, title: titleValue, assetDirectory }) {
  ...
  const buffer = await Packer.toBuffer(new Document({ creator: 'AIWordTranslateService9527', title, description: '由 AIWordTranslateService9527 ver0.2 生成', sections: [{ properties: {}, children }] }));
...
export async function createBundleExport(input) {
  ...
  if (selected.manifest) zip.file('manifest.json', JSON.stringify({ version: 'ver0.2', title, model: input.model || 'deepseek-chat', targetLanguage: input.targetLanguage, ruleSets: meta.ruleSets || [], sourceCharacters: source.length, translatedCharacters: translation.length, exportedAt: new Date().toISOString() }, null, 2));
