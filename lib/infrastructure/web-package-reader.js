...
export class WebPackageReader {
  ...
  async read(buffer) {
    ...
    if (buffer.length > this.limits.archiveBytes) throw problem('ZIP 文件不能超过 200 MB。', 'WEB_PACKAGE_ARCHIVE_TOO_LARGE', 413);
    let zip;
...
function isAllowedEntry(name) { return ['manifest.json', 'document.json', 'content.html'].includes(name) || /^assets\/[A-Za-z0-9._-]+\.(?:png|jpe?g|gif|webp|svg)$/i.test(name); }
