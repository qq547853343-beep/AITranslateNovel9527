...
const webPackageUpload = multer({ storage: multer.memoryStorage(), limits: { fileSize: 20 * 1024 * 1024, files: 1, fields: 0 } });
...
export async function createApplication({ baseDirectory, dataDirectory: suppliedDataDirectory, databaseFile, fetchImpl = fetch, secretStore: suppliedSecretStore, remoteImageImporter: suppliedRemoteImageImporter, webPackageImporter: suppliedWebPackageImporter, launcherControl = {} } = {}) {
  ...
  const webPackageImporter = suppliedWebPackageImporter || new ImportWebPackage({ reader: new WebPackageReader(), assetStore: new StagedAssetStore({ assetDirectory }) });
  ...
  return { app, database, repositories, secretStore, translationService, ruleProposalService, legacyTranslationService, webPackageImporter };
}
