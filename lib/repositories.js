...
export class RuleSetRepository {
  ...
  #validateRule(rule) {
    ...
    if (!rule.target && !['background','format','forbidden'].includes(rule.type)) throw Object.assign(new Error('规则目标内容不能为空。'), { status: 400 });
    if (rule.type === 'forbidden' && !rule.target && !strings(rule.forbidden).length) throw Object.assign(new Error('禁止译法规则必须填写目标译法或至少一个禁止译法。'), { status: 400 });
  }
...
export class TranslationJobRepository {
  ...
  create(input) {
    ...
    this.database.prepare(`INSERT INTO translation_jobs(id,title,target_language,model,rule_set_ids_json,validation_mode,status,source_blocks_json,segments_json,result_blocks_json,meta_json,total_segments,completed_segments,current_segment,created_at,updated_at,expires_at,revision,worker_id,lease_until,last_heartbeat_at,last_error,recovery_reason) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
      .run(id, input.title || 'translation', input.targetLanguage || '中文', input.model || 'deepseek-chat', JSON.stringify(input.ruleSetIds || []), input.validationMode || 'warning', input.status || 'pending', JSON.stringify(input.sourceBlocks || []), JSON.stringify(segments), JSON.stringify(input.resultBlocks || []), JSON.stringify(input.meta || {}), segments.length, 0, 0, timestamp, timestamp, input.expiresAt || new Date(Date.now() + 7 * 86400000).toISOString(), Number(input.revision) || 0, input.workerId || null, input.leaseUntil || null, input.lastHeartbeatAt || null, String(input.lastError || ''), String(input.recoveryReason || ''));
    return this.getById(id);
  }
  getById(id) { const row = this.database.prepare('SELECT * FROM translation_jobs WHERE id=?').get(id); return row ? hydrateJob(row) : null; }
  update(id, changes = {}, { expectedRevision, expectedWorkerId } = {}) {
    const current = this.getById(id); if (!current) throw Object.assign(new Error('翻译任务不存在。'), { status: 404 });
    const revision = expectedRevision == null ? current.revision : Number(expectedRevision);
    const next = { ...current, ...changes }; const completed = next.segments.filter((item) => item.status === 'completed').length;
    const workerClause = expectedWorkerId == null ? '' : ' AND worker_id=?';
    const result = this.database.prepare(`UPDATE translation_jobs SET status=?,segments_json=?,result_blocks_json=?,meta_json=?,completed_segments=?,current_segment=?,worker_id=?,lease_until=?,last_heartbeat_at=?,last_error=?,recovery_reason=?,revision=revision+1,updated_at=? WHERE id=? AND revision=?${workerClause}`)
      .run(next.status, JSON.stringify(next.segments), JSON.stringify(next.resultBlocks || []), JSON.stringify(next.meta || {}), completed, Number(next.currentSegment) || 0, next.workerId || null, next.leaseUntil || null, next.lastHeartbeatAt || null, String(next.lastError || ''), String(next.recoveryReason || ''), now(), id, revision, ...(expectedWorkerId == null ? [] : [expectedWorkerId]));
    if (!Number(result.changes)) throw jobConcurrencyError();
    return this.getById(id);
  }
  claim(id, workerId, { leaseMs = 120000, timestamp = Date.now() } = {}) {
    const current = this.getById(id); if (!current) return null;
    const heartbeat = new Date(timestamp).toISOString(); const leaseUntil = new Date(timestamp + leaseMs).toISOString();
    const result = this.database.prepare(`UPDATE translation_jobs SET status='running',worker_id=?,lease_until=?,last_heartbeat_at=?,last_error='',revision=revision+1,updated_at=? WHERE id=? AND revision=? AND (status='pending' OR (status='running' AND (lease_until IS NULL OR lease_until<=?)))`)
      .run(workerId, leaseUntil, heartbeat, heartbeat, id, current.revision, heartbeat);
    return Number(result.changes) ? this.getById(id) : null;
  }
  heartbeat(id, workerId, { leaseMs = 120000, timestamp = Date.now() } = {}) {
    const heartbeat = new Date(timestamp).toISOString(); const leaseUntil = new Date(timestamp + leaseMs).toISOString();
    const result = this.database.prepare(`UPDATE translation_jobs SET lease_until=?,last_heartbeat_at=?,updated_at=? WHERE id=? AND status='running' AND worker_id=?`)
      .run(leaseUntil, heartbeat, heartbeat, id, workerId);
    return Number(result.changes) > 0;
  }
  recoverExpired(id, { timestamp = Date.now(), reason = 'lease-expired' } = {}) {
    const current = this.getById(id); if (!current) return null;
    const at = new Date(timestamp).toISOString();
    const segments = current.segments.map((segment) => segment.status === 'translating' ? { ...segment, status: 'pending', requestId: null, startedAt: null, finishedAt: null, lastError: '', error: '' } : segment);
    const result = this.database.prepare(`UPDATE translation_jobs SET status='pending',segments_json=?,worker_id=NULL,lease_until=NULL,last_heartbeat_at=NULL,recovery_reason=?,revision=revision+1,updated_at=? WHERE id=? AND revision=? AND status='running' AND (lease_until IS NULL OR lease_until<=?)`)
      .run(JSON.stringify(segments), String(reason), at, id, current.revision, at);
    return Number(result.changes) ? this.getById(id) : null;
  }
