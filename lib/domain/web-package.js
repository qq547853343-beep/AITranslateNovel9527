...
const assetPathPattern = /^assets\/[A-Za-z0-9._-]+\.(?:png|jpe?g|gif|webp)$/i;
...
export function validateWebPackageDocument(input) {
  ...
  if (!Array.isArray(input.blocks) || input.blocks.length < 1 || input.blocks.length > 500) throw invalid('document.json 必须包含 1 到 500 个内容块。', 'WEB_PACKAGE_BLOCK_COUNT_INVALID');
  ...
  const blocks = input.blocks.map((block, index) => {
    ...
    if (block.kind === 'image') {
      assertOnlyKeys(block, ['id', 'kind', 'assetPath', 'alt', 'width', 'height'], `document.json.blocks[${index}]`);
      ...
      return { id, kind: 'image', assetPath: block.assetPath, alt: String(block.alt || '').slice(0, 200), width: boundedInteger(block.width, 900), height: boundedInteger(block.height, 600) };
    }
